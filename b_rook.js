class bRook{
    ...
    dohvati_mogucnosti_slijeva(){
        ...
        for(let i=0; i<svi_lijevo.length; i++){
            if(crne_figure.some(figura=>figura===svi_lijevo[i].innerText)){
                return mogucnosti
            }
    ...
    dohvati_mogucnosti_zdesna(){
        ...
        for(let i=0; i<svi_desno.length; i++){
            if(crne_figure.some(figura=>figura===svi_desno[i].innerText)){
                return mogucnosti
            }
    ...
    dohvati_mogucnosti_iznad(){
        ...
        for(let i=0; i<svi_iznad.length; i++){
            if(crne_figure.some(figura=>figura===svi_iznad[i].innerText)){
                return mogucnosti
            }
    ...
    dohvati_mogucnosti_ispod(){
        ...
        for(let i=0; i<svi_ispod.length; i++){
            if(crne_figure.some(figura=>figura===svi_ispod[i].innerText)){
                return mogucnosti
            }
    ...
    dodajListener(){
        ...
        this.trenutno_polje.addEventListener("click",()=>{
            if( this.trenutno_polje.innerText==="♜" && !is_kliknuto_crne_figure && !ciji_je_potez && this.dohvati_mogucnosti().length>0 ){
                ...
                if(this.trenutno_polje===document.querySelector("#a8")) is_black_a_rook_moved=true
                if(this.trenutno_polje===document.querySelector("#h8")) is_black_h_rook_moved=true
                mogucnosti.forEach(mogucnost=>{
                    ...
                    mogucnost.addEventListener("click",()=>{
                        if(is_clicked===false){
                            ...
                            let new_pawn = new bRook(mogucnost)
                            this.trenutno_polje.innerText=""
...
let BlackA_Rook= new bRook(document.querySelector("#a8"))
let BlackH_Rook= new bRook(document.querySelector("#h8"))
